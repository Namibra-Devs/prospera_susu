...
if ($total_data > 0) {
	...
	foreach ($result as $row) {
        ...
        // save row for later display
        $savingsData[] = $row;

        // if this savings entry was generated from advance payment
        if (!empty($row['saving_advance_id']) && $row['saving_advance_id'] != null) {
            $advanceIDs[] = $row['saving_advance_id'];
        }
        $advanceIDs = array_unique($advanceIDs);

        $advanceData = [];
        if (!empty($advanceIDs)) {
            $ids = implode(",", $advanceIDs);
            $sqlAdv = "SELECT * FROM saving_advance WHERE advance_id IN ($ids)";
            $stmtAdv = $conn->prepare($sqlAdv);
            $stmtAdv->execute();
            $advResults = $stmtAdv->fetchAll();
            foreach ($advResults as $adv) {
                $advanceData[$adv['id']] = $adv;  // store using ID as key
            }
        }
        ...
        // check if savings is having advance id
        if ($row['type'] == 'saving' && !empty($row['saving_advance_id']) && $row['saving_advance_id'] != null) {
            // group rows with same advance id together into one rows with a tag name "advance" 
            
            $output .= '
                <td colspan="10" class="table-info">
                    <strong>Advance Payment Transaction Group (Advance ID: ' . $row['saving_advance_id'] . ')</strong>
                    </td>
                </tr>';
